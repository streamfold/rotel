...
from rotel_sdk.open_telemetry.logs.v1 import LogRecord, ResourceLogs
...
class AttributeProcessor:
    ...
    def process_log_record(self, log_record: LogRecord):
        ...
        for action_kv in self._actions:
            ...
            if action_kv.action == Action.DELETE:
                ...
                if action_kv.compiled_regex:  # If a regex pattern is provided
                    keys_to_delete = self._get_matching_keys(action_kv.compiled_regex, log_record.attributes)
                ...
                if not target_kv:  # Only insert if key does not exist
                    ...
                    if source_value is not None:
                        log_record.attributes.append(KeyValue(action_kv.key, AnyValue(source_value)))
                ...
                if action_kv.compiled_regex:
                    keys_to_hash = self._get_matching_keys(action_kv.compiled_regex, log_record.attributes)
                ...
                for extracted_kv in extracted:
                    log_record.attributes.append(extracted_kv)
        ...
        log_record.attributes = final_attrs
...
processor_config = Config(
    actions=[
        ...
        # UPSERT with from_attribute: Copy 'user.email' to 'email_hash'
        ...
        # DELETE with regex: Remove any attribute ending with '.id'
...
def process(resource_logs: ResourceLogs):
    for log_record in itertools.chain.from_iterable(
        ...
        processor.process_log_record(log_record)
