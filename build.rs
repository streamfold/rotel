...
fn main() -> Result<()> {
    ...
    println!("cargo::rustc-check-cfg=cfg(node_metrics_integration_tests, values(\"true\"))");
    ...
    println!("cargo::rerun-if-env-changed=NODE_METRICS_INTEGRATION_TESTS");
    ...
    let env = std::env::var("NODE_METRICS_INTEGRATION_TESTS").unwrap_or("false".to_string());
    println!("cargo:rustc-cfg=node_metrics_integration_tests=\"{}\"", env);
